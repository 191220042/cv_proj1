...
def my_conv2d_numpy(image: np.ndarray, filter: np.ndarray) -> np.ndarray:
    ...
    fk, jf = filter.shape
    ## 计算padding的大小
    padding1 = ((fk - 1) // 2, (fk - 1) // 2 )
    padding2 = ((jf - 1) // 2, (jf - 1) // 2 )
    ## 创建一个新的图像
    filtered_image = np.zeros((m, n, c))
    ## 对图像进行padding
    image = np.pad(image, (padding1, padding2, (0, 0)), 'constant', constant_values=0)
    ## 对图像进行卷积
    for i in range(m):
        for j in range(n):
            for k in range(c):
                filtered_image[i][j][k] = np.sum(image[i:i + fk , j:j + jf , k] * filter)
...
def create_hybrid_image(
    ...
    blurry_image1 = my_conv2d_numpy(image1, filter)
    low_frequencies = blurry_image1

    blurry_image2 = my_conv2d_numpy(image2, filter)
    high_frequencies = image2 - blurry_image2

    hybrid_image = low_frequencies + high_frequencies
    
    ## 将hybrid_image的值限制在0-1之间
